"""
Optimized pipeline with caching and parallel support.
"""
import sys
import time
import os
...
try:
    from core.transcription_optimized import transcribe_audio_optimized
    HAS_OPTIMIZED = True
except ImportError:
    print("⚠️  faster-whisper not installed, using standard whisper")
    from core.transcription import transcribe_audio as transcribe_audio_optimized
    HAS_OPTIMIZED = False

try:
    from core.transcription_parallel import transcribe_audio_parallel
    HAS_PARALLEL = True
except ImportError:
    HAS_PARALLEL = False
...
def main(audio_path, num_speakers=None, min_speakers=None, max_speakers=None, 
    ...
    # Convert video if needed (convert_to_audio is defined in this module)
    ...
    if trans_cache and os.path.exists(paths["transcription"]):
        ...
        if use_parallel and HAS_PARALLEL:
            ...
            # transcribe_audio_optimized is set to core.transcription.transcribe_audio
            # when faster-whisper is not available, so call that wrapper.
            transcribe_audio_optimized(
                audio_path,
                paths["transcription"],
                chunk_length_minutes=chunk_minutes,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Optimized Pipeline")
