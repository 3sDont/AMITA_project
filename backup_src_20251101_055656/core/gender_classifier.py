"""Gender classification based on voice pitch."""
...
def classify_gender(diarization_path, output_path, audio_path=None):
    ...
    Args:
        diarization_path: File diarization result
        output_path: File output JSON
        audio_path: File audio gốc (để trích xuất audio từng speaker)
    """
    ...
    if audio_path and os.path.exists(audio_path):
        print(f"   🎵 Phân tích đặc trưng âm thanh từ: {audio_path}")

        # Load diarization segments
        diar_segments = load_diarization_segments(diarization_path)

        for speaker in sorted(speakers):
            print(f"   🎤 Xử lý {speaker}...")

            # Trích xuất audio của speaker
            speaker_audio, sr = extract_speaker_audio(audio_path, diar_segments, speaker)

            if speaker_audio is None or len(speaker_audio) < sr * 0.5:  # Ít nhất 0.5s
                print(f"      ⚠️  Không đủ audio để phân tích → Unknown")
            ...
            print(f"      ✅ Kết quả: {gender}")

    else:
        # Fallback: Dự đoán dựa trên pattern (không chính xác)
        print("   ⚠️  Không có audio gốc, sử dụng heuristic...")
        for i, speaker in enumerate(sorted(speakers)):
            # Heuristic đơn giản: xen kẽ Male/Female
    ...
    print(f"✅ Đã lưu kết quả giới tính vào {output_path}")
